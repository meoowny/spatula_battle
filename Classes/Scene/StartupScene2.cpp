...
bool StartupScene::init()
{
	...
	//设置游戏名标签
	auto labelGameName = Label::createWithSystemFont("金铲铲之战", "STHUPO.ttf", 150);
	labelGameName->setPosition(visibleSize.width / 2, visibleSize.height * 2 / 3);//将游戏名标签放置屏幕中间偏上
...
Menu* StartupScene::addMenuItem()
{
	Size visibleSize = Director::getInstance()->getVisibleSize();//获得屏幕大小
	//设置联机模式标签
	auto labelOnlineMode = Label::createWithSystemFont("联机模式", "STHUPO.ttf", 100);
	...
	//设置单机模式标签
	auto labelStandaloneMode = Label::createWithSystemFont("单机模式", "STHUPO.ttf", 100);
	...
	//添加退出游戏标签
	auto labelMenuClose = Label::createWithSystemFont("退出游戏", "STHUPO.ttf", 100);
